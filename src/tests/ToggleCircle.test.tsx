...
test("renders single enabled ToggleCircle component", () => {
    ...
    expect(option.childNodes[0]).toHaveClass('transition', 'transform', 'scale-100', 'fill-current', 'text-violet-600');
});
...
test("renders single DISabled ToggleCircle component", () => {
    ...
    const option = screen.getByLabelText("Vertical Slider Circle");
    expect(option.childNodes[0]).toHaveClass('transition', 'transform', 'scale-100', 'fill-current', 'text-white');
});
